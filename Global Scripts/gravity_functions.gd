		...
		for node in grav_zones_flipped:
			if node.get_priority() == priority_list.max(): #checks the priority of the node against the highest one available
				if node.is_gravity_a_point() and node.gravity_path == null:
					...
					#gravity_directions.append(_get_gravity_point_from_path(node.transformed_curve ,node.gravity_path, phys_node))
					gravity_directions.append(_get_gravity_point_from_path(node.gravity_path, phys_node))
				else:
...
func _get_gravity_point_from_path(path: Path3D, node: Node3D) -> Vector3:
	...
	curve_point = path.curve.sample_baked(offset_pos) * path.scale + (path.global_position - node.global_position) 
	return curve_point.normalized()

#func _get_gravity_point_from_path(path: Curve3D, w_node: Node3D, node: Node3D) -> Vector3:
	#var curve_point: Vector3
	#var relative_pos = _get_local_position(w_node, node)
	#var offset_pos: float = path.get_closest_offset(relative_pos)
	#curve_point = path.sample_baked(offset_pos) + (w_node.global_position - node.global_position)
	#return curve_point.normalized()
...
#func transform_curve_points(curve: Curve3D, scaler: Node3D) -> Curve3D:
	#var c_curve: Curve3D
	#var c_point: int = 0
	#while c_point < curve.get_point_count():
		##print(c_curve.get_point_position(c_point))
		##print(c_curve.get_point_position(c_point) * scaler.scale)
		##print(curve.get_point_position(c_point))
		#c_curve.set_point_position(c_point, ((curve.get_point_position(c_point) * scaler.scale)))
		#print(curve.get_point_position(c_point))
		#c_curve.set_point_in(c_point, curve.get_point_in(c_point) * scaler.scale)
		#c_curve.set_point_out(c_point, curve.get_point_out(c_point) * scaler.scale)
		#c_point += 1
	#return c_curve
